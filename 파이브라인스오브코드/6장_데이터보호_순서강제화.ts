...
class Transfer2 {
  ...
  /* 
  amount는 생성자에서 정의되고 이후 depositHelper 메소드에 직접적으로 참조되며 사용된다.
   1. 데이터 일관성: 클래스의 모든 메소드가 동일한 amount 값을 사용하므로, 데이터가 변화하는 과정에서 발생할 수 있는 오류를 줄일 수 있다.
   2. 코드 재사용성과 간결성: amount 값을 필드로 사용함으로써, 각 메소드가 동일한 값을 필요로 할 때 이를 파라미터로 전달하지 않아도 된다. 
   */
